...
my $isHeader = $ARGV[4];
my $outFile = $ARGV[5];
my $outFilePairs = $ARGV[6];


open(FTW,">$outFile") or die;
open(FTW_1,">$outFilePairs") or die;
...
my %athShannon = ();
my %fescShannon = ();
my %athFescExonVectorDist = ();
my %athFescEuclidDist = ();
my %athFescIdent = ();
...
sub LoadDataFromTabularFile
{
    ...
    open(FTR,"<$fileName") or die("$fileName");
